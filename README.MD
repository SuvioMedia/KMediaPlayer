...
- **Media3** for Android
- **kmedia-wasm-engine 0.4.0-alpha.4** for Kotlin/Wasm

## Project Status

This is a public, source-visible, proprietary SuvioMedia-maintained derivative used for internal projects and integrations. Repository visibility is not an open-source grant. GitHub users retain the limited platform rights required by the GitHub Terms of Service, including GitHub-hosted forking; use outside those rights is governed by [LICENSE](LICENSE). Immutable tagged artifacts are distributed only through Maven Central; GitHub Releases, GitHub Pages, and JitPack are not publication channels. Artifact availability is not an additional grant. Proprietary publications use empty source-JAR placeholders; `verifyClosedSourcePublications` blocks a release if any Maven source artifact contains implementation code. If you need a supported open-source library, use the upstream project instead: [kdroidFilter/ComposeMediaPlayer](https://github.com/kdroidFilter/ComposeMediaPlayer).

Published binaries embed the proprietary license, an artifact-scoped upstream Compose Media Player MIT copyright/license, and a component notice under `META-INF`. The release gate verifies that the repository and embedded copies of the proprietary license are identical before publication.
...
> § Wasm links the exact proprietary `io.github.shusek:kmedia-wasm-engine:0.4.0-alpha.4` Kotlin/Wasm KLIB. Its matching runtime-assets ZIP is packaged transitively as Compose resources, so playback does not import executable code from a CDN. The separate native runtime preserves upstream attribution and ships notices, corresponding source, build recipes and relinking instructions for its LGPL FFmpeg/Wasm payload. KMediaPlayer mounts the surface returned by the engine and never owns a parallel HTML-video playback route.
...
The current `4.1.x` desktop line is verified with the HDR fork Nucleus `2.4.0-kmp-hdr.4`
and Compose Multiplatform `1.11.1`.
...
The reviewed MPV runtime accepts local files plus direct `http` and `https`
sources on Android, iOS, macOS, Linux, and Windows. Request headers are passed as
a structured libmpv string array and are cleared before each source change:

```kotlin
playerState.openUri(
    uri = mediaUrl,
    requestHeaders = mapOf("Referer" to playbackReferer),
)
```

TLS peer verification is always enabled. Apple uses SecureTransport, Windows
uses SChannel, and Android/Linux use the reviewed OpenSSL runtime with a CA PEM
derived from the platform/JVM trust store. Private PKI can provide an
app-private absolute PEM path through
`MpvPlaybackOptions(tlsCertificateAuthorityFile = ...)`; there is no option to
disable certificate verification.

On Android, the bundled MPV runtime defaults to `gpu-next` with the `androidvk`
Vulkan context and changes the swapchain colorspace for each loaded source.
`VideoPlayerState.colorPipelineStatus` combines libmpv's decoded color and
mastering metadata with the producer-configured `ANativeWindow` dataspace and
pixel format. The adapter confirms HDR only for a PQ or HLG Surface backed by
10-bit RGBA or FP16; it confirms SDR independently after source transitions.
HDR10 and HLG are controlled native-range outputs. HDR10+ is reported as an
HDR10-compatible output with dynamic metadata dropped, and Dolby Vision is not
claimed as native output by this route.
...
separate macOS window. Public projection and texture-crop transforms stay on
the same MoltenVK route: KMediaPlayer injects a dynamic libplacebo `OUTPUT`
hook into `gpu-next`, so view motion, stereo windows and projection parameters
do not rebuild or replace the Vulkan video output. OpenGL remains only the
runtime fallback when macvk or the projection hook is rejected. The bundled
runtime must carry MoltenVK and KMediaMpv's versioned macvk capability to keep
the default route on Vulkan.
...
bundled runtime exposes only the reviewed `file`, `http`, and `https` input
surface (plus the internal protocols required by FFmpeg for those transports);
RTSP and RTMP are deliberately not enabled. `JvmHttpHlsMediaProxyFactory`
remains available as a source-adaptation tool for non-MPV desktop backends, but
MPV no longer needs a loopback proxy for normal HTTP(S) playback.
...
libass is not loaded as an unrelated application plugin: KMediaMpv links against
the same version-pinned KMediaFfmpegRuntime/KMediaAssRuntime graph already shared
by the MPV and KMediaBridge adapters.
...
The adapter contains no copied KMediaVlc implementation. It uses the same
backend-neutral `AbstractBackendVideoPlayerState` as MPV and talks to the
separately published LGPL-2.1-or-later KMediaVlc runtime through its ISC client
ABI. KMediaPlayer's public repository visibility does not change the runtime's
source, notice, replacement, or relinking rights.
...
ARMv7 devices. The Compose host supplies an opaque video SurfaceView plus a transparent media-overlay
SurfaceView for hardware-decoded subtitles. Automatic decoding requests an HDR-capable activity
window; KMediaVlc keeps MediaCodec directly on the final video Surface so BT.2020/PQ buffer metadata
is not lost through an intermediate layer. This HDR10 path and the two-Surface lifecycle are covered
on physical HDR hardware. The adapter accepts local paths, `file:`, `content:`, and HTTP(S) sources
and exposes automatic or software-only decoding through `LibVlcAndroidDecodeMode`. The Android probe
loads the native graph and checks its path-free build identity before creating a player. Explicit
...
The `4.1.18` adapter catalog targets KMediaVlc `0.1.0-rc.6` and normal builds resolve it from
Maven Central. The published-adapter consumer check can still use a local composite explicitly
while developing an unpublished KMediaVlc change; that check runs in two Gradle invocations:
...
|-----------------|----------------|-----------------|------|-------------------|
| 4.1.x           | 2.4.10         | 1.11.1          | 25   | AGP 9.3.1 / compileSdk 37 / minSdk 23; MPV/libVLC minSdk 28 |
...
The release workflow first publishes every module into a runner-local staging repository and validates the complete coordinates, POM metadata, and empty proprietary source JARs. It then performs the irreversible Maven Central publication and waits for every expected artifact to become available. Do **not** rerun publication for a version already accepted by Maven Central.
...
Wasm links the proprietary typed `io.github.shusek:kmedia-wasm-engine:0.4.0-alpha.4` KLIB directly.
The public API surface, Compose controls, overlays and fullscreen integration stay in KMediaPlayer,
while source routing, browser media elements, decoding, clocks and rendering stay in the engine. Matching
