...
- **Media3** for Android
- **movi-player 0.4.0-alpha.1 for Kotlin/Wasm** for WASM, with native HTML5 video available as an explicit non-adaptive compatibility mode
...
Wasm links the typed `io.github.shusek:movi-player:0.4.0-alpha.1` KLIB directly. The public state,
Compose surface, controls, overlays and fullscreen API stay unchanged. The matching Emscripten
`movi.js` and `movi.wasm` files come from `movi-player-runtime-assets` and are packaged transitively
in KMediaPlayer's Compose resources; there is no dynamic player-module import or Kotlin/JS facade.
