...
| Linux JVM | GStreamer `waylandsink` in a renderer-owned GTK drawing widget mounted directly by Nucleus Tao after GStreamer 1.28.5+, `color-management-v1`, compositor, and output negotiation pass | optional Vulkan renderer targets the widget's `wl_surface`; linear P010 DMA-BUF is mapped and uploaded, projected in linear BT.2020, then presented to an exact 10-bit PQ/HLG WSI colorspace. Compose controls remain in Tao's overlay-capable scene above the widget | any GTK/Tao, GStreamer, Wayland, DMA-BUF, Vulkan, renderer-library, or output negotiation failure selects the safe fallback; explicit `FORCE_SDR` uses a manifest-confirmed KMediaBridge bridge when available; `REQUIRE_HDR` reports the missing requirement |
...
Linux now uses one renderer-owned GTK drawing widget per native player surface. Nucleus Tao
reparents it into the window's GTK hierarchy and drives its bounds from Compose layout; realization
supplies the `wl_display`/`wl_surface` to GStreamer or the optional Vulkan renderer. Compose controls
remain above the widget, while Tao's input-region and native-popup plumbing preserves interaction.
The old JBR `WLSubSurface`, `wl_shm` overlay, and JAWT adapters no longer exist. Weston plus GNOME/KDE
input, resize, popup, and assistive-technology verification remains part of the hardware matrix.
