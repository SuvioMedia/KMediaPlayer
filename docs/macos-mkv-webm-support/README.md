...
The Linux libVLC native-view backend is deliberately X11/XWayland today. It creates a GTK drawing widget, Nucleus Tao reparents that `GtkWidget*` into its native window, and the widget's XID is passed to `libvlc_media_player_set_xwindow`. A real native Wayland libVLC path still needs a stable `wl_surface` embedding API, so it remains separate from the color-managed GStreamer Wayland route.
